...
<html>
  <head>
    ...
    <title>${ applicationScope.title}--后台管理--动画管理</title>
  ...
  <body>
    ...
    <c:choose>
    	...
    	<c:otherwise>
    		<table border="1" rules="rows" width="95%" >
    			...
    			<c:forEach var="t" items="${ requestScope.list}" varStatus="s">
    				<tr>
    					<td>
   							<input type="checkbox"  name="user" value="${ t.donghuaid}">
   						</td>
    					<td>${ t.donghuaid}</td>
      ...
      <div id="fen">
    		...
    		<a href="admin.do?method=getContent&p=1&c=cartoon">首页</a>
    		<c:choose>
    			<c:when test="${ requestScope.p <= 1}">上一页</c:when>
    			<c:otherwise><a href="admin.do?method=getContent&c=cartoon&p=${ requestScope.p - 1}">上一页</a></c:otherwise>
    		</c:choose>
    		<c:choose>
    			<c:when test="${ requestScope.p>=requestScope.all_page}">下一页</c:when>
    			<c:otherwise><a href="admin.do?method=getContent&c=cartoon&p=${ requestScope.p + 1}">下一页</a></c:otherwise>
    		</c:choose>
    		<a href="admin.do?method=getContent&c=cartoon&p=${ requestScope.all_page}">尾页</a>&nbsp;
